...
function fulfillEntrySpawnedByRuntimePrefetch(now, fetchStrategy, rsc, isPartial, staleAt, segmentVaryParams, tree, entriesOwnedByCurrentTask) {
    ...
    if (ownedEntry !== undefined) {
        ...
        if (tree.isPage) { var vp = tree.varyPath, parts = []; while(vp) { parts.push((vp.id || '_') + ':' + (typeof vp.value === 'object' ? 'Fallback' : String(vp.value).substring(0,80))); vp = vp.parent; } console.log('[rsc-debug] writeSegment page:', parts.join(' > '), 'entryStatus:', possiblyNewEntry.status); }
