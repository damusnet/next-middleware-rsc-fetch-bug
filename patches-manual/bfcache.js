...
function updateBFCacheEntryStaleAt(varyPath, newStaleAt) {
    ...
    if (entry !== null) {
        var oldStaleAt = entry.staleAt; entry.staleAt = newStaleAt; console.log("[rsc-debug] updateBFCacheStaleAt old:", oldStaleAt, "new:", newStaleAt, "now:", Date.now(), "expired:", newStaleAt <= Date.now());
    }
}
