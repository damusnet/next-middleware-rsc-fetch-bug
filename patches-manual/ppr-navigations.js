...
function createCacheNodeForSegment(now, tree, seedRsc, metadataVaryPath, seedHead, freshness, dynamicStaleAt) {
    ...
    if (tree.isPage) { var vp = tree.varyPath, parts = []; while(vp) { parts.push((vp.id || '_') + ':' + (typeof vp.value === 'object' ? 'Fallback' : String(vp.value).substring(0,80))); vp = vp.parent; } console.log('[rsc-debug] readSegment page:', parts.join(' > '), 'hit:', segmentEntry !== null && segmentEntry.status === 2); }
